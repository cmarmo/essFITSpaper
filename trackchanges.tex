...
%DIF DEL ../essFITSpaper-1.0/ess_marmo.tex   Mon Mar  5 10:03:44 2018
%DIF ADD ess_marmo.tex                       Fri May 11 22:14:04 2018
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
analysis to a more automated and quantitative analysis.
Interoperability, openness of data formats, and shared processing techniques are 
a necessity to \DIFdelbegin \DIFdel{efficiently extract scientific information from }\DIFdelend \DIFaddbegin \DIFadd{allow efficient scientific analysis of }\DIFaddend the data and 
to guarantee the reproducibility of the \DIFdelbegin \DIFdel{scientific }\DIFdelend results.
...
\DIFdelbegin \DIFdel{This paper is organized as follows:
%DIFDELCMD < \begin{enumerate}
%DIFDELCMD < \item{a brief description of FITS format;}
%DIFDELCMD < \item{a proposition for planetary surface metadata description in FITS;}
%DIFDELCMD < \item{dictionaries between FITS, PDS metadata description and EPN-TAP2.0
%DIFDELCMD < parameters;}
%DIFDELCMD < \item{some examples of planetary dataset in FITS;}
%DIFDELCMD < \item{current developments for making FITS more usable to handle data
%DIFDELCMD < of planetary surfaces.}
1. a brief description of FITS format;
2. a proposition for planetary surface metadata description in FITS;
3. dictionaries between FITS, PDS metadata description and EPN-TAP2.0 parameters;
4. some examples of planetary dataset in FITS;
5. current developments for making FITS more usable to handle data of planetary surfaces.}  \DIFdelend
...
transfer and archiving of telescope data by astronomical observatories\DIFaddbegin \DIFadd{, where
it has been used during the last thirty years}\DIFaddend .
It had been \DIFdelbegin \DIFdel{used }\DIFdelend \DIFaddbegin \DIFadd{adopted }\DIFaddend for data exchange and archiving from several orbital telescopes
\DIFaddbegin \DIFadd{and spatial missions}\DIFaddend .
...
\DIFaddbegin \DIFadd{Some Rosetta data, from NAVCAM and OSIRIS cameras, are distributed in FITS format at
the ESA Planetary Science Archive}\footnote{\DIFadd{https://archives.esac.esa.int/psa}} \DIFadd{(PSA).
}\DIFaddend In addition, many PDS3 datasets in the Small Bodies Node are archived and distributed
...
and vector data and, fortunately, it also offers basic support for the FITS format\DIFaddbegin \DIFadd{,
however missing a FITS metadata dictionary}\DIFaddend . 
...
\begin{table}
\caption{Projection names and codes \DIFdelbeginFL \DIFdelFL{already }\DIFdelendFL available in \DIFaddbeginFL \DIFaddFL{WCS }\DIFaddendFL FITS\DIFaddbeginFL \DIFaddFL{, }\DIFaddendFL suitable for
...
\DIFdelbeginFL %DIFDELCMD < \multicolumn{3}{l}{$^{b}$From the PDS4 Data Dictionary\footnote{https://pds.jpl.nasa.gov/pds4/doc/dd/current/PDS4\_PDS\_DD\_1900.pdf}. We cannot find correspondences in the WCS scheme for PDS4 Robinson,} \\
%DIFDELCMD < \multicolumn{3}{l}{Van Den Grinten and Space Oblique Mercator: those three projections are not used for archival or analysis.}
%DIFDELCMD < %%%
\DIFdelendFL
...
\begin{enumerate}
%DIFDELCMD < \item{RADESYS.
%DIFDELCMD < Name of the inertial reference frame with respect to which the coordinate
%DIFDELCMD < system is defined (defaulted to \texttt{`ICRS'})}
%DIFDELCMD < \item{EQUINOX.
%DIFDELCMD < The value field contains the equinox in years for the celestial
%DIFDELCMD < coordinate system in which positions are expressed (defaulted to \texttt{`J2000'})}
%DIFDELCMD < \item{WCSAXES.
%DIFDELCMD < Number of axes in the WCS description.
%DIFDELCMD < The value of WCSAXES may exceed the number of raster pixel axes. WCSAXES is, e.g.,
%DIFDELCMD < generally set to $2$ in two-dimensional rasters, it could be set to $3$ if an
%DIFDELCMD < additional time dependency is added to the coordinate description.}
%DIFDELCMD < \item{WCSNAME.
%DIFDELCMD < Name of world coordinate descriptions (e.g. body-fixed rotating -ocentric -ographic,
%DIFDELCMD < non-rotating, inertial).
%DIFDELCMD < EPN-TAP provides a \texttt{spatial\_coordinate\_description} parameter.}
%DIFDELCMD < %%%
\item{RADESYS.
Name of the inertial reference frame with respect to which the coordinate
system is defined\DIFdelbegin \DIFdelFL{ (defaulted to \texttt{`ICRS'})}\DIFdelendFL.
\DIFaddbegin \DIFaddFL{ Standard FITS defaults this keyword to \texttt{`ICRS'}.
...
This is the case starting from the 2003 WGCCRE report [Seidelmann et al., 2005] %\citep{wgccre2003}.
Before the 1982 WGCCRE report [Davies et al., 1983] %\citep{wgccre1982}
the inertial system was \texttt{`FK4'}, after it was \texttt{`FK5'}.
} \DIFaddendFL
}
\item{EQUINOX.
The value field contains the equinox in years for the celestial
coordinate system in which positions are expressed\DIFdelbegin \DIFdelFL{ (defaulted to \texttt{`J2000'})}\DIFdelendFL.
\DIFaddbegin \DIFaddFL{ Standard FITS defaults this keyword to \texttt{`J2000'}.
This is applicable to planetary data starting from the 1982 WGCCRE report [Davies et al., 1983].%\citep{wgccre1982}.
 Before 1982 the standard equinox was \texttt{`B1950'}.} \DIFaddendFL}
\item{WCSAXES.
...
additional time \DIFaddbegin \DIFaddFL{or filter} \DIFaddendFL dependency is added to the coordinate description.}
\item{WCSNAME.
Name of world coordinate descriptions (e.g. body-fixed rotating -ocentric -ographic,
non-rotating, inertial).
EPN-TAP provides \DIFdelbegin \DIFdelFL{a} \DIFdelendFL \DIFaddbegin \DIFaddFL{similar information through the} \DIFdelendFL \texttt{spatial\_coordinate\_description} parameter.}
...
%DIFDELCMD < \item{CDELTn.
%DIFDELCMD < The coordinate increment along axis $n$ (scaling component of the linear transformation
%DIFDELCMD < matrix PCi\_j).
%DIFDELCMD < In PDS4 they correspond to the \textit{pixel\_resolution\_x} and
%DIFDELCMD < \texttt{pixel\_resolution\_y} attributes.
%DIFDELCMD < In EPN-TAP they are connected to the \texttt{spatial\_resolution} parameter.}
%DIFDELCMD < \item{CUNITn.
%DIFDELCMD < Units of the coordinate system, not mandatory, defaulted to degrees for angular
%DIFDELCMD < coordinates and to meters (m) for linear ones.}
%DIFDELCMD < %%%
\item{CDELTn.
The coordinate increment along axis $n$ (scaling component of the linear transformation
matrix PCi\_j).
\DIFdelbegin \DIFdelFL{In EPN-TAP they are connected to} \DIFdelend \DIFaddbegin \DIFaddFL{They correspond to the} \DIFaddend \textit{pixel\_resolution\_x} and
\texttt{pixel\_resolution\_y} attributes in PDS4, to the \texttt{spatial\_resolution} parameter
\DIFaddbegin \DIFaddFL{in EPN-TAP} \DIFaddend.}
\item{CUNITn.
Units of the coordinate system \DIFdelbegin \DIFdelFL{, not mandatory, defaulted to degrees for angular coordinates and to meters (m) for linear ones}. \DIFdelend \DIFaddbegin \DIFaddFL{
This is a not mandatory keyword: when not present FITS standard
...
Codes for planets are \DIFdelbeginFL \DIFdelFL{assumed }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{implied }\DIFaddendFL in \citet{calagreisenI}.
...
%DIFDELCMD < \item{\texttt{ORIGIN}.
%DIFDELCMD < It contains string identifying the organization or institution responsible for
%DIFDELCMD < creating the FITS file.}
%DIFDELCMD < %%%
\item{\texttt{ORIGIN}.
It contains string identifying the organization or institution responsible for
creating the FITS file.
\DIFaddbegin \DIFadd{No clear correspondence for this reserved FITS keyword is available in PDS4.
...
As an example we have downloaded \DIFdelbegin \DIFdel{some global maps from }\DIFdelend \DIFaddbegin \DIFadd{a Mercury polar map from the }\DIFaddend USGS
PDS Annex\footnote{https://astrogeology.usgs.gov/search} and converted \DIFdelbegin \DIFdel{them }\DIFdelend \DIFaddbegin \DIFadd{it }\DIFaddend in FITS.
\DIFaddbegin \DIFadd{The FITS image has been cropped in two smaller versions using the open source software
SWarp}\footnote{\DIFadd{https://www.astromatic.net/software/swarp}} \DIFadd{\mbox{%DIFAUXCMD
\citep{swarp}}\hspace{0pt}%DIFAUXCMD
: SWarp was able to
automatically manage WCS planetary information.
}\DIFaddend We used ds9 to display the \DIFdelbegin \DIFdel{FITS image }\DIFdelend \DIFaddbegin \DIFadd{three FITS images }\DIFaddend and were able to \DIFdelbegin \DIFdel{visualize }\DIFdelend \DIFaddbegin \DIFadd{correctly align them.
Linear }\DIFaddend coordinates in meters \DIFaddbegin \DIFadd{are visualized (see figure \ref{fig:ds9qgis}, left side)
}\DIFaddend as in QGIS, a popular open source GIS software, \DIFdelbegin \DIFdel{but also in planetary latitude and
longitude }\DIFdelend thanks to the alternate coordinate
description\DIFdelbegin \DIFdel{(see figure \ref{fig:ds9qgis}, left side)}\DIFdelend .
\begin{figure}[ht!]
\centerline{\includegraphics[height=17pc]{ess_marmo2} \includegraphics[height=17pc]{ess_marmo1}}
\caption{left: Mercury South Pole FITS \DIFdelbeginFL \DIFdelFL{image }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{images }\DIFaddendFL displayed in ds9\DIFaddbeginFL \DIFaddFL{, alternate coordinates in meters
are displayed}\DIFaddendFL ;
right: GeoTIFF raster (displayed in \DIFdelbeginFL \DIFdelFL{yellow}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{red}\DIFaddendFL ) and the \DIFaddbeginFL \DIFaddFL{two }\DIFaddendFL virtual \DIFdelbeginFL \DIFdelFL{header }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{haeders
}\DIFaddendFL linked to \DIFdelbeginFL \DIFdelFL{the }\DIFdelendFL FITS \DIFdelbeginFL \DIFdelFL{raster }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{rasters }\DIFaddendFL (displayed in \DIFdelbeginFL \DIFdelFL{red}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{green and blue}\DIFaddendFL ); the \DIFdelbeginFL \DIFdelFL{two }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{three }\DIFaddendFL layers
...
GDAL Virtual Header\DIFaddbegin \footnote{\DIFadd{http://www.gdal.org/gdal\_vrttut.html}}
...
OMEGA unprojected and uncalibrated data (level1b) are distributed by ESA-PSA \DIFdelbegin \DIFdel{and PDS
archives}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citep{BESSE2018131}}\hspace{0pt}%DIFAUXCMD
}\DIFaddend .
...
\DIFaddbegin \DIFadd{The complete implementation of FITS metadata in GDAL library is in progress.
}\DIFaddend Once implemented in GDAL, FITS will officially become a supported format by QGIS.

In addition to that, a QGIS plugin has been developed\footnote{https://github.com/epn-vespa/VO\_QGIS\_plugin}
\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\citep[][in preparation]{Minin} }\hspace{0pt}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citep{Minin} }\hspace{0pt}%DIFAUXCMD
}\DIFaddend in order to download and display files exposed via EPN-TAP servers.
...
Currently, \DIFdelbegin \DIFdel{it }\DIFdelend \DIFaddbegin \DIFadd{MATISSE }\DIFaddend provides access to public data from five different targets (i.e., Mercury,
Venus, Mars, \DIFaddbegin \DIFadd{the }\DIFaddend dwarf planet Ceres and \DIFaddbegin \DIFadd{the }\DIFaddend asteroid Vesta)\DIFdelbegin \DIFdel{using both data stored on SSDC local
...
One of th }\DIFdelend \DIFaddbegin \DIFadd{.
}\DIFaddend MATISSE 2D outputs \DIFdelbegin \DIFdel{is }\DIFdelend \DIFaddbegin \DIFadd{are }\DIFaddend formatted as FITS, \DIFdelbegin \DIFdel{projected by interpolating the
original observations to a grid regularly spaced
in planetocentric coordinates (i.
e., latitude and longitude).
We used the }\DIFdelend \DIFaddbegin \DIFadd{in equirectangular projection and in planetocentric coordinates.
The }\DIFaddend planetary FITS standard described in this work \DIFdelbegin \DIFdel{in }\DIFdelend \DIFaddbegin \DIFadd{has been implemented in MATISSE in }\DIFaddend order to make the output
...
online by the end of 2018) \DIFdelbegin \DIFdel{we are planning to make use of }\DIFdelend JS9 \DIFdelbegin \DIFdel{as }\DIFdelend \DIFaddbegin \DIFadd{will be }\DIFaddend the standard reader for
...
\DIFdelbegin \DIFdel{It }\DIFdelend \DIFaddbegin \DIFadd{Aladin }\DIFaddend is able to load FITS files and to display images taking into account various
WCS projections.
\DIFdelbegin \DIFdel{Like DS9, Aladin is popular and broadly used in the part of the astronomical
community manipulating images.
}\DIFdelend So far it is also the reference generator and visualizer of Hierarchical
...
\DIFdelbegin \DIFdel{For example, it will be able to take into account a planet's radii to compute
distances on the surface of the body.
}\DIFdelend Finally, among the WCS projections listed in Tab. \ref{table:proj}, several of
...
Aladin.
\DIFdelbegin \DIFdel{We expect all projections to be available in a forthcoming release.
}\DIFdelend 
...
\DIFdelbegin \DIFdel{We are aware that FITS is today subject of questioning in astronomical applications}
\DIFdelend \DIFaddbegin \DIFadd{We understand that some believe FITS has grown too old for newer astronomical
applications} \DIFaddend \citep{THOMAS2015133}.
We are not convinced that one format can solve all the issues faced from long-term
archiving to on-line streaming.
\DIFdelbegin \DIFdel{Planetary science research community has used a large variety of format customizing
specific software.
...
allowing easy conversion for final products following the needed application.} \DIFdelend
\DIFaddbegin \DIFadd{But we want to stress that using FITS can be an efficient compromise.
...
archival products.} \DIFaddend

FITS \DIFdelbegin \DIFdel{as }\DIFdelend \DIFaddbegin \DIFadd{is }\DIFaddend an open and flexible data and metadata exchange and archiving format, \DIFdelbegin \DIFdel{could be
inserted }\DIFdelend \DIFaddbegin \DIFadd{its use
}\DIFaddend in the PDS4 data model scheme \DIFdelbegin \DIFdel{in order to address }\DIFdelend \DIFaddbegin \DIFadd{would solve }\DIFaddend the issue of \DIFdelbegin \DIFdel{more }\DIFdelend complex
metadata and data relationships \DIFdelbegin \DIFdel{.
}\DIFdelend \DIFaddbegin \DIFadd{separating data processing and the data archiving processes.
}\DIFaddend 

\DIFdelbegin \DIFdel{In order to improve interoperability and knowledge sharing between astronomy and
planetary science, we will soon finalize the }\DIFdelend \DIFaddbegin \DIFadd{The }\DIFaddend integration of FITS specifications in the GDAL library \DIFaddbegin \DIFadd{will be finalized soon}\DIFaddend .
...
%DIFDELCMD < %%%
\DIFdelend %\bibliography{ess_marmo}
...
\DIFaddbegin \bibitem[{\textit{{Bertin} et~al.}(2002)\textit{{Bertin}, {Mellier},
  {Radovich}, {Missonnier}, {Didelon}, and {Morin}}}]{swarp}
{\DIFadd{Bertin}}\DIFadd{, E., Y.~}{\DIFadd{Mellier}}\DIFadd{, M.~}{\DIFadd{Radovich}}\DIFadd{, G.~}{\DIFadd{Missonnier}}\DIFadd{, P.~}{\DIFadd{Didelon}}\DIFadd{, and
  B.~}{\DIFadd{Morin}} \DIFadd{(2002), }{\DIFadd{The TERAPIX Pipeline}}\DIFadd{, in }\textit{\DIFadd{Astronomical Data
  Analysis Software and Systems XI}}\DIFadd{, }\textit{\DIFadd{Astronomical Society of the
  Pacific Conference Series}}\DIFadd{, vol. 281, edited by D.~A. }{\DIFadd{Bohlender}}\DIFadd{,
  D.~}{\DIFadd{Durand}}\DIFadd{, and T.~H. }{\DIFadd{Handley}}\DIFadd{, p. 228.
}
...
\DIFadd{Besse, S., C.~Vallat, M.~Barthelemy, D.~Coia, M.~Costa, G.~D. Marchi, D.~Fraga,
  ...
  sets, }\textit{\DIFadd{Planetary and Space Science}}\DIFadd{, }\textit{\DIFadd{150}}\DIFadd{, 131 -- 140,
  }\doi{https://doi.org/10.1016/j.pss.2017.07.013}\DIFadd{, enabling Open and
  Interoperable Access to Planetary Science and Heliophysics Databases and
  Tools.
}

\DIFaddend \bibitem[{\textit{{Bibring} et~al.}(2005)\textit{{Bibring}, {Langevin},
...
\DIFdelbegin %DIFDELCMD < \bibitem[{\textit{Erard et~al.}(2014)\textit{Erard, Cecconi, Sidaner, Berthier,
%DIFDELCMD <   Henry, Molinaro, Giardino, Bourrel, Andr̩, Gangloff, Jacquey, and
%DIFDELCMD <   Topf}}]{ERARDepntap}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \bibitem[{\textit{{Davies} et~al.}(1983)\textit{{Davies}, {Abalakin}, {Lieske},
  {Seidelmann}, {Sinclair}, {Sinzi}, {Smith}, and {Tjuflin}}}]{wgccre1982}
{\DIFadd{Davies}}\DIFadd{, M.~E., V.~K. }{\DIFadd{Abalakin}}\DIFadd{, J.~H. }{\DIFadd{Lieske}}\DIFadd{, P.~K. }{\DIFadd{Seidelmann}}\DIFadd{, A.~T.
  }{\DIFadd{Sinclair}}\DIFadd{, A.~M. }{\DIFadd{Sinzi}}\DIFadd{, B.~A. }{\DIFadd{Smith}}\DIFadd{, and Y.~S. }{\DIFadd{Tjuflin}} \DIFadd{(1983), }{\DIFadd{Report
  of the IAU working group on cartographic coordinates and rotational elements
  of the planets and satellites - 1982}}\DIFadd{, }\textit{\DIFadd{Celestial Mechanics}}\DIFadd{,
  }\textit{\DIFadd{29}}\DIFadd{, 309--321, }\doi{10.1007/BF01228525}\DIFadd{.
}
...
\bibitem[{\textit{Erard et~al.}(2014)\textit{Erard, Cecconi, Sidaner, Berthier,
  ...
  Topf}}]{ERARDepntap}
\DIFaddend Erard, S., B.~Cecconi, P.~L. Sidaner, J.~Berthier, F.~Henry, M.~Molinaro,
  M.~Giardino, N.~Bourrel, N.~\DIFdelbegin \DIFdel{Andr̩}\DIFdelend \DIFaddbegin \DIFadd{Andr}{\DIFadd{\'e}}\DIFaddend , M.~Gangloff, C.~Jacquey, and F.~Topf
...
\DIFdelbegin %DIFDELCMD < \bibitem[{\textit{{Minin} et~al.}(2018)\textit{{Minin}, {et al.}, and {et
%DIFDELCMD <   al.}}}]{Minin}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \bibitem[{\textit{{Minin} et~al.}(2018)\textit{{Minin}, {Rossi}, {Marco
  Figuera}, {Unnithan}, Marmo, Walter, Demleitner, Le~Sidaner, Cecconi, S., and
  Hare}}]{Minin}
\DIFaddend {Minin}, M., \DIFaddbegin \DIFadd{A.~}\DIFaddend {\DIFdelbegin \DIFdel{et al.}\DIFdelend \DIFaddbegin \DIFadd{Rossi}\DIFaddend }, \DIFdelbegin \DIFdel{and }\DIFdelend \DIFaddbegin \DIFadd{R.~}\DIFaddend {\DIFdelbegin \DIFdel{et al.}\DIFdelend \DIFaddbegin \DIFadd{Marco Figuera}\DIFaddend }\DIFaddbegin \DIFadd{, V.~}{\DIFadd{Unnithan}}\DIFadd{, C.~Marmo,
  S.~Walter, M.~Demleitner, P.~Le~Sidaner, B.~Cecconi, E.~S., and T.~Hare
  }\DIFaddend (2018)\DIFaddbegin \DIFadd{, }{\DIFadd{Bridging the gap between Geographical Information Systems and
  Planetary Virtual Observatory}}\DIFadd{, }\textit{\DIFadd{this issue}}\DIFaddend .
...
{Pence}, W.~D., L.~{Chiappetti}, C.~G. {Page}, R.~A. {Shaw}, and E.~{Stobie}
  ...
  \doi{10.1051/0004-6361/201015362}\DIFaddbegin \DIFadd{.
}
...
{\DIFadd{Seidelmann}}\DIFadd{, P.~K., B.~A. }{\DIFadd{Archinal}}\DIFadd{, M.~F. }{\DIFadd{A'Hearn}}\DIFadd{, D.~P. }{\DIFadd{Cruikshank}}\DIFadd{,
  J.~L. }{\DIFadd{Hilton}}\DIFadd{, H.~U. }{\DIFadd{Keller}}\DIFadd{, J.~}{\DIFadd{Oberst}}\DIFadd{, J.~L. }{\DIFadd{Simon}}\DIFadd{, P.~}{\DIFadd{Stooke}}\DIFadd{,
  D.~J. }{\DIFadd{Tholen}}\DIFadd{, and P.~C. }{\DIFadd{Thomas}} \DIFadd{(2005), }{\DIFadd{Report of the IAU/IAG Working
  Group on Cartographic Coordinates and Rotational Elements: 2003}}\DIFadd{,
  }\textit{\DIFadd{Celestial Mechanics and Dynamical Astronomy}}\DIFadd{, }\textit{\DIFadd{91}}\DIFadd{, 203--215,
  }\doi{10.1007/s10569-004-3115-4}\DIFaddend .
