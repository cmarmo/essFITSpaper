...
%DIF DEL ../essFITSpaper-2.0/ess_marmo.tex   Sat May 12 18:39:23 2018
%DIF ADD ess_marmo.tex                       Wed Aug 29 22:33:00 2018
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
in the \DIFaddbegin \DIFadd{Planetary Data System (}\DIFaddend PDS4\DIFaddbegin \DIFadd{) }\DIFaddend metadata archival model and lastly provide example 
implementations.
...
This paper proposes to use \DIFdelbegin \DIFdel{Flexible Image Transport
System}\DIFdelend \DIFaddbegin \DIFadd{FITS}\DIFaddend \footnote{https://fits.gsfc.nasa.gov/}
\DIFdelbegin \DIFdel{(FITS) }\DIFdelend format \citep{fitsorig,fitsver3} in planetary surface investigations,
...
The International Astronomical Union \DIFaddbegin \DIFadd{(IAU) }\DIFaddend approved FITS as the standard format
...
Observatory\DIFdelbegin \DIFdel{(VO)}\DIFdelend .
...
\DIFdelbegin \DIFdel{Finally, it }\DIFdelend \DIFaddbegin 

\DIFadd{FITS format }\DIFaddend is supported by a large number of open libraries
and software tools\DIFaddbegin \DIFadd{, developed in numerous languages.
The reference C and Fortran library, CFITSIO}\footnote{\DIFadd{https://heasarc.gsfc.nasa.gov/docs/software/fitsio/fitsio.html}}\DIFadd{,
...
other languages}\footnote{\DIFadd{A non exhaustive list is available at https://fits.gsfc.nasa.gov/fits\_libraries.html}}\DIFadd{.
Software tools for visualization and analysis make use of those libraries or implement
their own, having different levels of maturity and option completness}\footnote{\DIFadd{See https://fits.gsfc.nasa.gov/fits\_viewer.html}}\DIFaddend .
...
the ESA Planetary Science Archive\footnote{\DIFdelbegin \DIFdel{https}\DIFdelend \DIFaddbegin \DIFadd{http}\DIFaddend ://\DIFdelbegin \DIFdel{archives}\DIFdelend \DIFaddbegin \DIFadd{psa}\DIFaddend .\DIFdelbegin \DIFdel{esac.}\DIFdelend esa.int\DIFdelbegin \DIFdel{/psa}\DIFdelend } (PSA).
In addition, many PDS3 datasets in the Small Bodies Node are archived and distributed
as FITS files with PDS3 labels.
\DIFdelbegin \DIFdel{However, to be }\DIFdelend \DIFaddbegin 

\DIFadd{However, solar system imagery data have been traditionally described using terrestrial based
...
\DIFadd{With the impressive increasing of raster data from planetary missions, the efficient
...
\DIFadd{To be }\DIFaddend more efficiently used in planetary surface investigations, FITS metadata
must be \DIFaddbegin \DIFadd{thouroughly mapped to planetary geospatial concepts.
Then, they must be }\DIFaddend extended in order to take into account the size of the reference body and
...
In the framework of the \DIFdelbegin \DIFdel{VESPA}\DIFdelend \DIFaddbegin \DIFadd{Virtual European Solar and Planetary Access}\DIFaddend \footnote{http://europlanet-vespa.eu/} \DIFaddbegin \DIFadd{(VESPA)
}\DIFaddend \citep{vespapss} workpackage of the EuroplanetRI2020\footnote{http://www.europlanet-2020-ri.eu/}
...
FITS World Coordinate System \DIFaddbegin \DIFadd{(WCS)
}\DIFaddend representation\footnote{http://fits.gsfc.nasa.gov/fits\_wcs.html} is the standard
...
products using Equirectangular projection \DIFdelbegin \DIFdel{(CAR) }\DIFdelend on a local surface, approximated
...
\DIFdelbegin %DIFDELCMD < \item{CTYPEn.
%DIFDELCMD < Name of the coordinate represented by axis $n$, in ``4--3'' form: the first four
%DIFDELCMD < characters specify the coordinate type, the fifth character is a `-' and the
%DIFDELCMD < remaining three characters specify an algorithm code for computing the world coordinate
%DIFDELCMD < value (\ref{table:proj}).
%DIFDELCMD < For angular coordinates \citet{calagreisenI} provides the form xLN xLT or yzLN yzLT,
%DIFDELCMD < where x or yz correspond to the target body.
%DIFDELCMD < We propose to standardize those codes as listed in Table \ref{table:ctype}.
%DIFDELCMD < For projected coordinates in meters we propose a similar scheme: xPX as Projected X,
%DIFDELCMD < xPY as Projected Y, or yzPX and yzPY.
%DIFDELCMD < Alternative representation of WCS are already possible in FITS, to describe images in
%DIFDELCMD < degrees and in meters \citep{greisencalaII}.
%DIFDELCMD < The name of the alternative axis will be specified using the alternative CNAMEn
%DIFDELCMD < keyword.}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \item{CTYPEn.
...
\DIFdelbegin %DIFDELCMD < \item{CDELTn.
%DIFDELCMD < The coordinate increment along axis $n$ (scaling component of the linear transformation
%DIFDELCMD < matrix PCi\_j).
%DIFDELCMD < They correspond to the \textit{pixel\_resolution\_x} and
%DIFDELCMD < \texttt{pixel\_resolution\_y} attributes in PDS4, to the \texttt{spatial\_resolution} parameter
%DIFDELCMD < in EPN-TAP.}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \item{CDELTn.
...
be used (meters, m).}
\DIFdelbegin %DIFDELCMD < \item{CNAMEn.
%DIFDELCMD < The description of a particular coordinate for an axis in a particular WCS (up to 68
%DIFDELCMD < characters), while the WCSNAMEa keyword names the particular WCS as a whole.
%DIFDELCMD < Its default value will be blank.}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \item{CNAMEn.
...
OGCCODE	& string & reserved, optional & Value field contains a string describing the standard \DIFdelbegin \DIFdel{OGC }\DIFdelend \DIFaddbegin \DIFadd{Open Geospatial Consortium (OGC) }\DIFaddend code \\
...
\DIFdelbegin %DIFDELCMD < \item{\texttt{DATE-OBS}.
%DIFDELCMD < It contains the date of the observation, provided as ISO 8601
%DIFDELCMD < string. The value shall be assumed to refer to the start of the observation,
%DIFDELCMD < unless another interpretation is clearly explained in the comment field.
%DIFDELCMD < It is assumed to be expressed in UTC but other systems are accepted as long as they
%DIFDELCMD < are specified in the ISO string\footnote{See table 30 in the Standard FITS document https://fits.gsfc.nasa.gov/standard40/fits\_standard40aa.pdf}. 
%DIFDELCMD < PDS4 provides a \texttt{Time\_Coordinates} class with attributes
%DIFDELCMD < \texttt{start\_date\_time} and \texttt{stop\_date\_time} providing start and
%DIFDELCMD < stop date and time appropriate to the product being labeled, always in UTC.
%DIFDELCMD < The \texttt{time\_min} and \texttt{time\_max} fields are mandatory in the EPN-TAP
%DIFDELCMD < scheme, they must always be formatted in UTC as a ISO string (planned in EPN-TAP v2.1).}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \item{\texttt{DATE-OBS}.
...
\DIFdelbegin %DIFDELCMD < \item{\texttt{REFERENC}.
%DIFDELCMD < It contains a character string citing the bibliographic reference where the
%DIFDELCMD < data are first described.
%DIFDELCMD < It is recommended that either the Astrophysics Data System bibliographic
%DIFDELCMD < databases\footnote{http://adswww.harvard.edu/}
%DIFDELCMD < bibliographic identifier or the Digital Object Identifier\footnote{http://doi.org}
%DIFDELCMD < be used.
%DIFDELCMD < PDS4 \texttt{External\_Reference} class and EPN-TAP \texttt{bib\_reference} have
%DIFDELCMD < exactly the same meaning and use than the FITS keyword.}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \item{\texttt{REFERENC}.
...
As an example we have downloaded a Mercury polar map from the \DIFdelbegin \DIFdel{USGS
}\DIFdelend \DIFaddbegin \DIFadd{United States Geological Survey (USGS)
}\DIFaddend PDS Annex\footnote{https://astrogeology.usgs.gov/search} and converted it in FITS.
...
are displayed;
right: GeoTIFF raster (displayed in red) and the two virtual \DIFdelbegin \DIFdel{haeders
}\DIFdelend \DIFaddbegin \DIFadd{headers}\DIFaddend
...
\label{sec:softdev}
\DIFaddbegin \DIFadd{In section \ref{ssec:geofits} the mapping between planetary metadata concepts and
...
On remote sensing side, our goal is to get FITS into habits of geospatial developers,
improving GDAL and QGIS compatibility with FITS.
On the other side, planetary astronomers will benefit of a FITS efficient description
of their data, once the new keywords fully implemented in their traditional tools.
We provide here two examples chosen within the Europlanet VESPA collaboration.
}\DIFaddend 
...
and VO services.
The next step \DIFdelbegin \DIFdel{to VO and GIS tool integration }\DIFdelend will be to directly implement metadata translation into \DIFdelbegin \DIFdel{GDAL C++ library. 
}\DIFdelend \DIFaddbegin \DIFadd{the GDAL library. 
This work is already in progress, and new developments are available from the VESPA github
repository}\footnote{\DIFadd{https://github.com/epn-vespa/gdal}}\DIFadd{.
}\DIFaddend 
...
\DIFdelbegin \DIFdel{The complete implementation of FITS metadata in GDAL library is in progress.
}\DIFdelend Once implemented in GDAL, FITS will officially become a supported format by QGIS.
...
\subsection{MATISSE}
MATISSE\footnote{https://tools.\DIFdelbegin \DIFdel{asdc}\DIFdelend \DIFaddbegin \DIFadd{ssdc}\DIFaddend .asi.it/matisse.jsp} (Multi-purpose Advanced Tool
...
whose output is a \DIFdelbegin \DIFdel{VRT virtual GDAL header }\DIFdelend \DIFaddbegin \texttt{\DIFadd{.vrt}} \DIFadd{GDAL Virtual Header }\DIFaddend pointing to the FITS file with the appropriate
projection.
...
applications \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\citep{THOMAS2015133}}\hspace{0pt}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citep[e.g.][]{THOMAS2015133}}\hspace{0pt}%DIFAUXCMD
}\DIFaddend . We are also not convinced that one format
...
%DIF <  Definition here
%DIF <  \acro{EMOS}
%DIF <  Ensemble model output statistics 
%DIF <  \acro{ECMWF}
%DIF <  Centre for Medium-Range Weather Forecasts
%DIF <  \end{acronyms}
\DIFaddbegin \begin{acronyms}
\acro{AMICA}
\DIFadd{Asteroid Multi-band Imaging Camera
}\acro{BIL}
\DIFadd{Band Interleaved per Line
}\acro{BSQ}
\DIFadd{Band SeQuential
}\acro{CRISM}
\DIFadd{Compact Reconnaissance Imaging Spectrometer for Mars
}\acro{DOI}
\DIFadd{Digital Object Identifier
}\acro{EAICD}
\DIFadd{Experiment Archive Interface Control Document
}\acro{EPN-TAP}
\DIFadd{EuroPlaNet Table Access Protocol
}\acro{ESA}
\DIFadd{European Space Agency
}\acro{FITS}
\DIFadd{Flexible Image Transport System
}\acro{GDAL}
\DIFadd{Geospatial Data Abstraction Library
}\acro{GeoTIFF}
\DIFadd{Geographical Tagged Image File Format
}\acro{GIS}
\DIFadd{Geographical Information Systems
}\acro{HiPS}
\DIFadd{Hierarchical Progressive Survey
}\acro{HiRISE}
\DIFadd{High Resolution Imaging Science Experiment
}\acro{IAU}
\DIFadd{International Astronomical Union
}\acro{IEEE}
\DIFadd{Institute of Electrical and Electronics Engineers
}\acro{IVOA}
\DIFadd{International Virtual Observatory Alliance
}\acro{ISO}
\DIFadd{International Organization for Standardization
}\acro{JAXA}
\DIFadd{Japan Aerospace Exploration Agency
}\acro{MATISSE}
\DIFadd{Multi-purpose Advanced Tool for the Instruments for the Solar System Exploration
}\acro{MEF}
\DIFadd{Multi Extension FITS
}\acro{NAVCAM}
\DIFadd{NAVigation CAMera
}\acro{NIRS}
\DIFadd{Near-Infrared Spectrometer
}\acro{OGC}
\DIFadd{Open Geospatial Consortium
}\acro{OMEGA}
\DIFadd{Observatoire pour la Min\'eralogie, l'Eau, les Glaces et l'Activit\'e
}\acro{OSIRIS}
\DIFadd{Optical, Spectroscopic, and Infrared Remote Imaging System
}\acro{PDS}
\DIFadd{Planetary Data System
}\acro{PDS3}
\DIFadd{Planetary Data System (version 3)
}\acro{PDS4}
\DIFadd{Planetary Data System (version 4)
}\acro{PSA}
\DIFadd{Planetary Space Archive
}\acro{SSDC-ASI}
\DIFadd{Space Science Data Center of the Italian Space Agency
}\acro{USGS}
\DIFadd{United States Geological Survey
}\acro{UTC}
\DIFadd{Universal Time Coordinated
}\acro{VESPA}
\DIFadd{Virtual European Solar and Planetary Access
}\acro{VO}
\DIFadd{Virtual Observatory
}\acro{WCS}
\DIFadd{World Coordinate System
}\acro{WGCCRE}
\DIFadd{Working Group on Cartographic Coordinates and Rotational Elements
}\acro{WKT}
\DIFadd{Well Known Text
}\end{acronyms}
\DIFaddend 
