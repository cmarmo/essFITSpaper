...
\item{WCSNAME.
Name of world coordinate descriptions (e.g. body-fixed rotating -ocentric -ographic, non-rotating, inertial).
...
\item{CTYPEn.
Name of the coordinate represented by axis $n$, inn “4–3” form: the first four characters
...
For linear coordinates a similar scheme (xPX as Projected X xPY as Projected Y or yzPX and yzPY) is proposed.
Alternative representation of WCS are already possible in FITS, to describe images in degrees and in meters
\citep{greisencalaII}.
...
world coordinates.}
\item{PCi\_j.
The $ij$ linear transformation matrix (rotation, skewness) defining the intermediate pixel coordinates (to be used with CDELT keywords).}
\item{CDELTn.
